package com.lib.exo.statusView.ui
...
class HkStoryHorizontalProgressView : View {
    ...
    private var mHkStoryPlayerListener: HkStoryPlayerListener? = null
    ...
    companion object {
        const val HK_PROGRESS_BAR_HEIGHT = 2
        const val HK_GAP_BETWEEN_PROGRESS_BARS = 2
        const val HK_SINGLE_STORY_DISPLAY_TIME = 2000
        const val HK_PROGRESS_PRIMARY_COLOR = "#ebebe6"
        const val HK_PROGRESS_SECONDARY_COLOR = "#696868"
    }
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        val typedArray = context.obtainStyledAttributes(
            attrs, R.styleable.HkStoryHorizontalProgressView, 0, 0
        )

        try {
            resource?.let {
                mProgressHeight = typedArray.getDimension(
                    R.styleable.HkStoryHorizontalProgressView_hkProgressBarHeight,
                    HK_PROGRESS_BAR_HEIGHT.toFloat()
                ).toInt().dpToPx(it)

                mGapBetweenProgressBars = typedArray.getDimension(
                    R.styleable.HkStoryHorizontalProgressView_hkGapBetweenProgressBar,
                    HK_GAP_BETWEEN_PROGRESS_BARS.toFloat()
                ).toInt().dpToPx(it)

                mProgressBarPrimaryColor = typedArray.getColor(
                    R.styleable.HkStoryHorizontalProgressView_hkProgressBarPrimaryColor,
                    Color.parseColor(HK_PROGRESS_PRIMARY_COLOR)
                )

                mProgressBarSecondaryColor = typedArray.getColor(
                    R.styleable.HkStoryHorizontalProgressView_hkProgressBarSecondaryColor,
                    Color.parseColor(HK_PROGRESS_SECONDARY_COLOR)
                )

                mSingleStoryDisplayTime = typedArray.getInt(
                    R.styleable.HkStoryHorizontalProgressView_hkSingleStoryDisplayTime,
                    HK_SINGLE_STORY_DISPLAY_TIME
                ).toLong()
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun setDefaultValue() {
        resource?.let {
            mProgressHeight = HK_PROGRESS_BAR_HEIGHT.dpToPx(it)
            mGapBetweenProgressBars = HK_GAP_BETWEEN_PROGRESS_BARS.dpToPx(it)
            mProgressBarPrimaryColor = Color.parseColor(HK_PROGRESS_PRIMARY_COLOR)
            mProgressBarSecondaryColor = Color.parseColor(HK_PROGRESS_SECONDARY_COLOR)
            mSingleStoryDisplayTime = HK_SINGLE_STORY_DISPLAY_TIME.toLong()
        }
    }
     ...
     */
    private fun setHkStoryProgressBarCount(count: Int) {
    ...
    fun consumeAttrib(hashOfAttributeSet: HashMap<String, Any>, storyCount: Int) {
        setGapBetweenProgressBars(hashOfAttributeSet[HkStoryDisplayActivity.HK_FULLSCREEN_GAP_BETWEEN_PROGRESSBAR] as Int)
        setProgressBarHeight(hashOfAttributeSet[HkStoryDisplayActivity.HK_FULLSCREEN_PROGRESSBAR_HEIGHT] as Int)
        setProgressBarPrimaryColor(hashOfAttributeSet[HkStoryDisplayActivity.HK_FULLSCREEN_PROGRESSBAR_PRIMARY_COLOR] as Int)
        setProgressBarSecondaryColor(hashOfAttributeSet[HkStoryDisplayActivity.HK_FULLSCREEN_PROGRESSBAR_SECONDARY_COLOR] as Int)
        setHkStoryProgressBarCount(storyCount)
    }
    ...
    fun startAnimating(index: Int) {
        ...
        if (index >= mProgressbarCount) {
            if (mHkStoryPlayerListener != null) {
                mHkStoryPlayerListener?.onFinishedPlaying(isAtLastIndex = true)
                return
            }
        }
            ...
            addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    if (isPreviousStory.not())
                        mHkStoryPlayerListener?.onStoryFinished(index)
        ...
        mHkStoryPlayerListener?.onStartedPlaying(index)
    }
     ...
     */
    fun setHkStoryPlayerListener(hkStoryPlayerListener: HkStoryPlayerListener?) {
        this.mHkStoryPlayerListener = hkStoryPlayerListener
    }
    ...
    fun moveToNextStoryPoint() {
        ...
        if (currentIndex == mProgressbarCount - 1) {
            ...
            if (mProgressAnimators != null) {
                ...
                mHkStoryPlayerListener?.onStoryFinished(currentIndex) // Invoke onStoryFinished as soon as animation ends.
    ...
    fun moveToPreviousStoryPoint(moveToIndex: Int) {
        ...
        if (currentIndex == INITIAL_STORY_INDEX) {
            ...
            mHkStoryPlayerListener?.onFinishedPlaying(isAtLastIndex = false)
        } else {
     ...
     */
    interface HkStoryPlayerListener {
